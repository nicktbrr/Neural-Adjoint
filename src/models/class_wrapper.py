...
import pandas as pd
# Own module
from src.utils.time_recorder import time_keeper
from src.utils.helper_functions import simulator

class Network(object):
    def __init__(self, model_fn, flags, train_loader, test_loader,
                 ckpt_dir=os.path.join(os.getcwd(), 'src', 'models'),
    ...
    def train(self):
        ...
        with mlflow.start_run(run_name=self.flags.model_name or time.strftime('%Y%m%d_%H%M%S', time.localtime())):

            mlflow.log_params({
                    "learning_rate": self.flags.lr,
                    "batch_size": self.flags.batch_size,
                    "reg_scale": self.flags.reg_scale,
                    "train_step": self.flags.train_step,
                    "optimizer": self.flags.optim,
                    "data_set": self.flags.data_set
            })
            ...
            for epoch in range(self.flags.train_step):
                ...
                if epoch % self.flags.eval_step:                      # For eval steps, do the evaluations and tensor board
                    ...
                    if test_avg_loss < self.best_validation_loss:
                        ...
                        sample_input = next(iter(self.test_loader))[0][:1]
                        if cuda:
